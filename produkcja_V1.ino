...
//////////////stan baterii
double SOC;                 // poziom naładowania %; 2,5V==0; 4,2V==100%
...
void setup() {
  ...
  if (SD.exists("pojemnosc")) {
    ...
    if (dataFile) {
      mAh1=mAh_max;
      dataFile.println(mAh1);  ///jeśli pliku brak, nadpisz go wartością max (przy definicji)
      dataFile.close();
...
void bubbleEffect() {
  ...
  // Przesuń diody w górę z zanikiem koloru
  for (int i = NUM_LEDS - 1; i > 0; i--) {
    leds[i] = leds[i - 1];
    leds[i].fadeToBlackBy(40); // kolorowy ogon
  }

  // Dodaj nowy bąbelek u dołu
  if (bubbleCooldown == 0 && random8() < 50) {
    bubbleHue = CHSV(random8(), 255, 255);
    leds[0] = bubbleHue;
    bubbleCooldown = random8(5, 20);
  } else {
