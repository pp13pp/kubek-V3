...
mpu.begin();
  ...
  mpu.setFilterBandwidth(MPU6050_BAND_10_HZ);   //chyba częściej nie ma sensu
...
czas = currentMillis - previousMillis; // 
...
// if (changeState)
// {
//         changeState = false; //przerwanie
//         przelancznik=analogRead(wlancz)*3.3/1024*2.1; ///odczyt napiecia //*2.1<- dzielnik//1024==3,3V
        
//         if (przelancznik > 2) //powyżej 2V oznacza włącznik w pozycji ON i walisz zwarciem
//         { 
//             digitalWrite(laduj, LOW);
//         }
//         else if(termika==0 && ladowanie == 1) ///jak termicznie bezpiecznie, dopiero myśl o ładowaniu //żądanie ładowania =1; nie ładuj=0
//         {
//           if (przelancznik < 1)
//           {
...
Serial.print("termika ");
Serial.println(termika);
Serial.println("");
...
void pomiary_ele() ////pomiary elektryczne INA219
{
  ...
  busvoltage = ina219.getBusVoltage_V();     
  current_mA = ina219.getCurrent_mA()+0.6;  // prąd ładowania / rozładowywania (przy zerowym poborze wskazuje między -0,4 a -0,6mA)
...
if(current_mA<0.3) ///upewnić się że prąd w - oznacza ładowanie
{
  // if(laduj==0) ///jak masz prąd ładowania, a nie wystawiałeś sygnału do ładowania, daj zwrotkę o błędzie 
  // {
  // Serial.println("ładujesz a nie powinieneś");
  // Serial.println(" ");
